...
module {
    ...
    /// Check account
    public func validate(account : TokenTypes.Account) : Bool {
    ...
    public func encode({ owner; subaccount } : TokenTypes.Account) : TokenTypes.EncodedAccount {
    ...
    public func decode(encoded : TokenTypes.EncodedAccount) : ?TokenTypes.Account {
        ...
        if (bytes[size - 1] == 0x7f) {
            ...
            let principal = Principal.fromBlob(Blob.fromArray(slice(bytes, 0, split_index)));
    ...
    private func slice<T>(xs : [T], i : Nat, j : Nat) : [T] {
        if (j < i) return [];
        if (j == i) return [xs[i]];
